...
class AggTradeData:
    ...
    last_trade_id: int
    timestamp: int
    is_buyer_maker: bool  # m field: True = seller aggressed (hit bid), False = buyer aggressed (lifted ask)
...
class BinanceIndicatorFetcher:
    ...
    async def __aenter__(self):
        if self._session is None:
            timeout = aiohttp.ClientTimeout(
                total=self._config.timeout_total,
    ...
    async def _get_premium_index(self, symbol: str) -> Dict[str, Any]:
        ...
        return await self._get(f"{self.FUTURES_BASE}/fapi/v1/premiumIndex", {
            'symbol': symbol
        })
    ...
    async def _get(self, url: str, params: Optional[Dict] = None) -> Any:
        ...
        for attempt in range(self._config.max_retries):
            try:
                logger.debug("GET %s params=%s (attempt %d/%d)", url, params, attempt + 1, self._config.max_retries)
    ...
    async def get_funding_rate_history(
        ...
        data = await self._get(f"{self.FUTURES_BASE}/fapi/v1/fundingRate", {
            'symbol': symbol,
    ...
    async def get_open_interest(
        ...
        data = await self._get(f"{self.FUTURES_BASE}/fapi/v1/openInterest", {
            'symbol': symbol
        })
    ...
    async def get_open_interest_history(
        ...
        data = await self._get(f"{self.FUTURES_BASE}/futures/data/openInterestHist", {
            'symbol': symbol,
            'period': period,
            'limit': min(limit, 500)
        })
    ...
    async def get_long_short_ratio(
        ...
        data = await self._get(f"{self.FUTURES_BASE}/futures/data/topLongShortAccountRatio", {
            'symbol': symbol,
            'period': period,
            'limit': min(limit, 500)
        })
    ...
    async def get_taker_volume(
        ...
        data = await self._get(f"{self.FUTURES_BASE}/futures/data/takerlongshortRatio", {
            'symbol': symbol,
            'period': period,
            'limit': min(limit, 500)
        })
    ...
    async def validate_symbol(self, symbol: str, futures: bool = True) -> bool:
        ...
        try:
            ...
            if cached is None:
                base = self.FUTURES_BASE if futures else self.SPOT_BASE
                endpoint = f"{base}/fapi/v1/exchangeInfo" if futures else f"{base}/api/v3/exchangeInfo"
